...
var entries = []Entry{
	...
	},
	{
		...
		Variant:        "Exact token values with configurable minimum token and line spans; comments and import declarations are never tokens, and a window never spans an import declaration; regular expressions, template literal parts and JSX text are single opaque tokens.",
	...
	},
	{
		...
		Classification: Conventional,
		Definition:     "One block of tokens copied in two or more places, extended to the longest region the copies share, reported when clone density exceeds the project threshold. Its value is the number of lines the copies take up together.",
		Variant:        "Identified by the files and functions holding the copies; tokens as for duplication.token_clone_density.",
		Limitations:    "Evidence for review, not a mandate to abstract. A copy moved into another function looks new.",
